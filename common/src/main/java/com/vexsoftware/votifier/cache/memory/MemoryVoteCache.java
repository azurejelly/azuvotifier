package com.vexsoftware.votifier.cache.memory;
...
public class MemoryVoteCache implements VoteCache {
    ...
    public MemoryVoteCache(VotifierPlugin plugin, long voteTTL) {
        ...
        plugin.getScheduler().repeatOnPool(this::sweep, 12, 12, TimeUnit.HOURS);
    }
    ...
    private void sweep(Map<String, Collection<VoteWithRecordedTimestamp>> m) {
        for (Map.Entry<String, Collection<VoteWithRecordedTimestamp>> entry : m.entrySet()) {
            ...
            while (vi.hasNext()) {
                ...
                if (hasTimedOut(v)) {
                    logger.warn("Purging out of date vote.", v);
                    vi.remove();
                }
            }
        }
    ...
    public static class VoteWithRecordedTimestamp extends Vote {
        ...
        public VoteWithRecordedTimestamp(Vote vote) {
            super(vote);
            this.recorded = System.currentTimeMillis();
        }

        public VoteWithRecordedTimestamp(JsonObject object) {
            super(object);
